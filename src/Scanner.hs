...
data MDToken = T_Newline     -- '\n' 
             ...
             | T_SPACE Int -- Führende Leerzeichen
             | T_ITALIC  -- * 
             | T_BOLD  -- ** fett
             | T_SLASH  -- Escape zeichen
             | T_OpenSqu        -- [
             | T_CloseSqu       -- ]
             | T_Exclam         -- !
             | T_OpenBracket    -- (
...
textScan text str@('[':xs) =   (\tokens ->  ((textScan text "")++T_OpenSqu:tokens))     $ textScan "" xs
textScan text str@(']':xs) =   (\tokens ->  ((textScan text "")++T_CloseSqu:tokens))    $ textScan "" xs
...
textScan text str@('`':xs) =   (\tokens ->  ((textScan text "")++T_BackQuote:tokens))   $ textScan "" xs
textScan text str@(':':xs) =   (\tokens ->  ((textScan text "")++T_DoublePoint:tokens)) $ textScan "" xs
